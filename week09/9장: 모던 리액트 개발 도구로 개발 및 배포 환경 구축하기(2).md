...
- Serveless Function: 서버 없이 함수를 클라우드에 구축해 실행할 수 있다. 특이한 점은 Next.js에서 제공하는 /pages/api의 내용도 이 함수로 구분되어 접근 로그나 오류 등을 확인할 수 있다.
...
- `docker build`: Dockerfile을 기준으로 이미지를 빌드한다. -t 옵션으로 태그를 붙일 수 있다. 예를 들어, `docer build -t foo:bar ./` ./에 있는 Dockerfile을 기준으로 이미지를 빌드하고 해당 이미지 명에 foo:bar라는 태그를 붙인다.
...
- `COP . ./`: 빌드를 위해 src, node_modules 등 필요한 리소스를 복사한다.
...
여기서 눈여겨볼 만한 점은 httpCreateServer로 만든 서버에 NextServer를 연동한다. 이러한 점을 응용한다면 Koa나 Express 같은 웹 프레임워크에 Next.js를 올려 두고 실행해 별도의 Node.js 기반 서버를 운영하면서 동시에 Next.js도 서비스할 수 있을 것이다. 
